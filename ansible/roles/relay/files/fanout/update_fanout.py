...
CONF_DIR = "/opt/fanout/config"
UNIT_REGEXP = re.compile("^fanout@(.+)\.target$")
...
fanout_source={stream["transcoding"]["sink"]}
"""
...
def get_unitname(stream):
    return f"fanout@{stream['key']}.target"
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Fetch stream info and adapt local fanout tasks")
    parser.add_argument("url", help="streaming info url")
    parser.add_argument("name", help="local hostname")
