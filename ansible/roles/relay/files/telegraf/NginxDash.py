...
import os, re, datetime, collections, logging
...
def parse_logdate(datestr):
    ...
    datetime-object (doing it manually is faster then strptime)
    """
...
def count_dash_viewers():
    ...
    # - store each ip + ident which requested the manifest in a set to eliminate duplicates
    # - count the set keys
...
def main():
    viewers = list(count_dash_viewers().items())
    for stream, count in viewers:
        print(f"viewers,stream={stream},transport=dash total={count}")
