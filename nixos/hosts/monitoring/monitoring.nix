...
let
  ...
  scrapeConfigFile = import ./scrape-config.nix { inherit lib pkgs config vmalertPort; };
...
in
{
  ...
  environment.etc."victoriametrics/prometheus.yaml".source = scrapeConfigFile;
  systemd.services.victoriametrics.serviceConfig.ExecReload = "${pkgs.coreutils}/bin/kill -HUP $MAINPID";
  systemd.services.victoriametrics.reloadTriggers = [ scrapeConfigFile ];
