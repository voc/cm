...
#
# December 2015: Added multiple socket support. Based on 321a0dc [c3voc]
...
def get_stats():
  ...
  for socket in HAPROXY_SOCKET:
      ...
      try:
        ...
        logger('warn', "status err Unable to connect to HAProxy socket at %s" % socket)
        return socket_stats

      if 'server' in PROXY_MONITORS:
        for key,val in server_info.items():
          try:
            socket_stats[key] = int(val)
      ...
      for statdict in server_stats:
        ...
        for key,val in statdict.items():
          ...
          try:
            socket_stats[metricname] = int(val)
          except (TypeError, ValueError), e:
            pass
      stats.append(socket_stats)
...
def configure_callback(conf):
  ...
  for node in conf.children:
    if node.key == "ProxyMonitor":
      ...
      HAPROXY_SOCKET.append(node.values[0])
    elif node.key == "Verbose":
      VERBOSE_LOGGING = bool(node.values[0])
    else:
      logger('warn', 'Unknown config key: %s' % node.key)

  if len(HAPROXY_SOCKET) == 0:
...
def read_callback():
  ...
  for data in info:
    for key,value in data.items():
      ...
      if 'value' in key_values[key_name].keys():
        key_values[key_name]['value'] += value
      else:
        key_values[key_name]['value'] = value

  # log all collected keys
  logger('verb', 'keys: %s' % key_values.keys())

  for key,values in key_values.items():
    logger('verb', '%s: %s' % (key, values))
    val = collectd.Values(plugin=NAME, type=values['val_type'])
